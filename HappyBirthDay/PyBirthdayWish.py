...
def replaceMultiple(mainString: str, toBeReplace: list, newString):
...
def pprint(art, time):
    ...
    for i in range(len(art)):
        ...
        print(colored(replaceMultiple(art[i], colorCodes, ''), random.choice(
            color_used), attrs=colorAttribute), sep='', end='', flush=True)
        sleep(time)
...
def pcode():
    ...
    if codePrint:
        for i in range(len(code)):
            print(colored(code[i], codeColor), sep='', end='', flush=True)
            sleep(codingSpeed)
        input('\n\n'+colored('python3', 'blue') +
